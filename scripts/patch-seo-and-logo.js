...
const MARKER = "<!-- seo:injected:v2 -->";
const OLD_MARKERS = ["<!-- seo:injected -->"];

const PAGE_META = {
  "index.html": {
    path: "/",
    title: "3we — AI-First Open Platform for Embodied Robotics",
  },
  "product.html": {
    path: "/product",
    title: "3we Product — One API, Four Backends, Zero Compromise",
  },
  "hardware.html": {
    path: "/hardware",
    title: "3we Hardware — Open BOM, KiCad PCB, CERN-OHL-P v2",
  },
  "benchmarks.html": {
    path: "/benchmarks",
    title: "3we Benchmarks — Standardized Embodied AI Evaluation",
    desc: "3 tasks × 8 scenes × public leaderboard. Reproducible evaluation across simulation and real hardware.",
  },
  "playground.html": {
    path: "/playground",
    title: "3we Playground — Try the Platform in Your Browser",
    desc: "Interactive playground for the 3we Robot Platform on HuggingFace Spaces. No installation required.",
  },
  "blog.html": {
    path: "/blog",
    title: "3we Blog — Engineering Notes on Embodied AI",
  },
  "community.html": {
    path: "/community",
    title: "3we Community — Open Source Embodied AI Research",
  },
  "paper.html": {
    path: "/paper",
    title: "3we Paper — Sim-to-Real Embodied AI Research Platform",
  },
  "pricing.html": {
    path: "/pricing",
    title: "3we Pricing — Open Hardware Configurations from $120",
  },
  "about.html": {
    path: "/about",
    title: "About 3we — Mission, Team, and Open Licensing",
  },
  "docs.html": {
    path: "/docs",
    title: "3we Documentation — Redirecting to docs.3we.org",
    desc: "Technical documentation for the 3we Robot Platform. Redirecting to docs.3we.org.",
  },
};
...
<meta property="og:image:alt" content="3WE Robot Platform — AI-First Open Source Embodied Robotics"/>
<meta name="twitter:card" content="summary_large_image"/>
<meta name="twitter:title" content="${title}"/>
<meta name="twitter:description" content="${desc}"/>
<meta name="twitter:image" content="${SITE_URL}/img/og-image.png"/>
<meta name="twitter:image:alt" content="3WE Robot Platform — AI-First Open Source Embodied Robotics"/>`;
}

const DEFAULT_DESC =
  "AI-First open infrastructure for embodied robotics. Run the same Python code from simulation to real hardware. Open-source robot platform under $500.";
...
function stripOldSeoBlock(html) {
  // Remove any block previously injected by an older version of this script.
  // Match from the old marker through the last twitter:* meta we ever wrote.
  for (const oldMarker of OLD_MARKERS) {
    const idx = html.indexOf(oldMarker);
    if (idx === -1) continue;
    // Find the end of the previously-injected block: the last <meta name="twitter:image" ...> right after.
    const tail = html.slice(idx);
    const endMatch = tail.match(/<meta\s+name="twitter:image[^>]*>\s*/);
    if (!endMatch) continue;
    const endIdx = idx + endMatch.index + endMatch[0].length;
    html = html.slice(0, idx) + html.slice(endIdx);
    // Drop the leading newline left behind, if any.
    html = html.replace(
...
function patch(filePath) {
  ...
  if (!html.includes(MARKER)) {
    ...
    const meta = PAGE_META[fname] || {
      path: "/" + fname.replace(".html", ""),
      title: extractTitle(html) || "3we Robot Platform",
    };
